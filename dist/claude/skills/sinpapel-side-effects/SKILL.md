...
**El framework NO renderiza plantillas.** `Documento` describe la plantilla
(`plantilla` FileField, `contenido`, `tipo_plantilla` DOCX/PDF,
`configuracion_overlay`), pero el render lo implementas tú dentro del side
effect; sinpapel solo persiste el resultado en `archivo_generado`. Si el
render es lento, encólalo (Celery) y retorna rápido — recuerda que un fallo
del side effect **no** revierte la transición.
