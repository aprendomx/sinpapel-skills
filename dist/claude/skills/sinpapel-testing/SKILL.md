...
## Lo que NO ocurre dentro de una transacción de test

Tres mecanismos del framework se cuelgan de `transaction.on_commit()` o miran
la bitácora completa. En un test envuelto en transacción —el default de
`pytest.mark.django_db`— no se comportan como en producción, y el síntoma es
siempre el mismo: parece que la funcionalidad no existe.

**Los webhooks salientes no se emiten.** `emit_event` va en `on_commit`, y el
test nunca commitea:
...
**La invalidación del cache tampoco.** Los signals de `sinpapel.signals`
envuelven cada `cache.delete` en `on_commit`. De ahí la fixture autouse que
limpia el cache: sin ella, un `Estado` de un test anterior sobrevive con un id
de una fila ya revertida.

**El SLA toma como referencia el `fecha_accion` más reciente de TODA la
bitácora de la instancia**, no la última fila insertada. Para envejecer un
expediente hay que mover todas:
