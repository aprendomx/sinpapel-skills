...
class Trazable(models.Model):
    ...
    autor = models.ForeignKey(User, null=True, related_name="%(class)s_autor", on_delete=SET_NULL)
    modificador = models.ForeignKey(User, null=True, related_name="%(class)s_modificador", on_delete=SET_NULL)
...
Desde 0.8.0, `autor` y `modificador` usan **`SET_NULL`** (antes `CASCADE`):
borrar un `User` ya **no** borra en cascada sus registros — se preservan
con la autoría en `NULL` (migración `0007`).
...
- `firma_registro` (OneToOne nullable a `RegistroFirma`; desde 0.8.0 con
  `on_delete=PROTECT`).
...
**Advertencia**: los `queryset.update()` / `queryset.delete()` masivos
**NO** pasan por estos hooks del ORM — siguen pudiendo mutar o borrar
filas. No los uses sobre estas tablas.
