...
5. Edita transiciones (grupos, firma electrónica requerida, condiciones, requisitos) en el panel derecho. El toggle "Firma electrónica" mapea a `ConfiguracionTransicion.requiere_firma` (sinpapel 0.8.x) y hace round-trip en el JSON v0.2 (JSONs viejos sin la key importan con false).
